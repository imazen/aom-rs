//! Differential harness for the highbd directional intra predictors
//! z1_high / z2_high / z3_high vs C libaom v3.14.1
//! (`av1_highbd_dr_prediction_z{1,2,3}_c`), over valid angle-derived (dx, dy),
//! both upsample settings, and bitdepths {8,10,12}.

use aom_intra::dir::{get_dx, get_dy, z1_high, z2_high, z3_high, EdgeRef16};
...
fn edges(rng: &mut Rng, bw: usize, bh: usize, bd: i32) -> Vec<u16> {
...
fn highbd_dr_predictors_identical() {
        ...
        for &(bw, bh) in SIZES.iter() {
            for up in 0..2i32 {
                ...
                for angle in 1..90 {
                    ...
                    let above = edges(&mut rng, bw, bh, bd);
    ...
    assert!(checks > 9000, "expected many checks, got {checks}");
}
