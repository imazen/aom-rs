...
struct Rng(u64);
impl Rng { fn next(&mut self)->u64{let mut x=self.0;x^=x>>12;x^=x<<25;x^=x>>27;self.0=x;x.wrapping_mul(0x2545_F491_4F6C_DD1D)} }
    ...
    for &bd in &[10i32, 12] {
        ...
        for mode in 0..10usize {
            for (size_idx, &(bw, bh)) in SIZES.iter().enumerate() {
                for _ in 0..800 {
                    let above_tl: Vec<u16> = (0..bw+1).map(|_| (rng.next() % (maxv as u64 +1)) as u16).collect();
                    let left: Vec<u16> = (0..bh).map(|_| (rng.next() % (maxv as u64 +1)) as u16).collect();
                    let mut got = vec![0u16; bw*bh];
                    predict_highbd(mode, &mut got, bw, bw, bh, &AboveRef16(&above_tl), &left, bd);
                    let want = c::ref_highbd_intra_pred(mode, size_idx, bw, bh, &above_tl, &left, bd);
