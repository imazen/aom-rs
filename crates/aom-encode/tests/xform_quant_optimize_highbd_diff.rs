//! High-bit-depth, high-magnitude differential for the trellis
//! (`xform_quant_optimize` = av1_xform_quant + get_txb_ctx + av1_optimize_b) vs C.
//!
//! The existing `xform_quant_optimize_diff` locks the trellis at bd8 with
//! residuals in ±255. That never exercises the large-coefficient regime that
//! genuine 10/12-bit content produces: a ±4095 residual through a 32×32/64×64
//! forward transform yields coefficients in the hundreds of thousands, whose
//! squared distortion (the trellis' get_coeff_dist term) reaches ~1e10-1e11 —
//! past i32 range. This differential drives bd=12 with ±4095 residuals so any
//! integer-width divergence between the port trellis and C's int64_t math shows
...
#[test]
fn xform_quant_optimize_highbd_identical() {
    let mut rng = Rng(0x12b0_c0de_bd12_0007);
    const TX_TYPES: [usize; 7] = [0, 1, 2, 3, 9, 10, 11];
    let (mut total, mut nonzero_eob, mut reduced_eob, mut skipped) =
    ...
    for tx_size in 0..19usize {
        ...
        for &tx_type in &TX_TYPES {
            ...
            for iter in 0..40 {
                // Genuine high-bit-depth residual magnitude: ±4095 (12-bit).
                let residual: Vec<i16> = (0..full).map(|_| rng.range(-4095, 4096) as i16).collect();
                // Reciprocal quant/dequant (quant ~ 2^16/dequant) as the real
                // encoder tables are, so dqcoeff reconstructs ~tcoeff and the QM
                // trellis' squared (diff*qm) in get_coeff_dist stays realistic
                // (over-quantized dqcoeff, not realistic, is what blows it up).
                ...
                // Oracle: fwd -> HIGHBD quant -> get_txb_ctx -> optimize (or skip).
                ...
                let (mut qc, mut dqc, eob0) = match (kind, use_qm) {
                    ...
                    (QuantKind::B, false) => c::ref_highbd_quantize_b(
                        ls,
                        src,
                        &zbin,
                        &round,
                ...
                // Realistic-magnitude guard. libaom's get_coeff_dist documents that
                // tcoeff/dqcoeff are at most 22 bits, so `diff*diff` cannot overflow
                // int64. Uncorrelated random ±4095 residuals through large transforms
                // can exceed that bound — a regime real content never reaches and in
                // which C itself is undefined (int64 overflow). Skip those so the
                // trellis is exercised only across the magnitudes the real bd12
                // encoder actually produces (still far above the bd8 ±255 test).
                const COEFF_MAX: u32 = 1 << 21;
                let over = src
                    .iter()
                    .chain(dqc.iter())
                    .any(|&c| c.unsigned_abs() >= COEFF_MAX);
                if over {
                    skipped += 1;
                    continue;
                }
                ...
                let m =
                    format!("ts={tx_size} tt={tx_type} kind={kind:?} qm={use_qm} eob0={eob0} bd12");
                total += 1;
                if eob0 == 0 {
    ...
    eprintln!(
        "bd12 trellis: {total} in-range blocks ({nonzero_eob} nonzero-eob, \
         {reduced_eob} eob-reduced), {skipped} skipped as >22-bit (unrealistic)"
    );
    // Must have exercised a substantial body of realistic high-bd blocks, not
    // just skipped everything down to trivial cases.
    assert!(
        total >= 200,
        "too few in-range bd12 blocks exercised: {total}"
    );
    assert!(
        nonzero_eob * 4 >= total,
