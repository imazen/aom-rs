//! `av1/common/quant_common.c` — the DC/AC dequantizer lookup tables and the
//! `av1_{dc,ac}_quant_QTX` accessors. Bit-exact vs C (differential-tested over
//! all qindex × delta × bit_depth). These map a base qindex to the Q3-scaled
//! quantizer step that the encoder and decoder share.

/// `MAXQ` (`av1/common/quant_common.h`) — the top of the qindex range.
const MAXQ: i32 = 255;

// The tables are transcribed verbatim from `av1/common/quant_common.c` (the row
// grouping matches the C source so the transcription can be eyeballed against
// it). Correctness is proven exhaustively in `tests/dc_quant_diff.rs`.

#[rustfmt::skip]
static DC_QLOOKUP_QTX: [i16; 256] = [
...
#[rustfmt::skip]
static DC_QLOOKUP_10_QTX: [i16; 256] = [
...
#[rustfmt::skip]
static DC_QLOOKUP_12_QTX: [i16; 256] = [
...
#[rustfmt::skip]
static AC_QLOOKUP_10_QTX: [i16; 256] = [
...
#[rustfmt::skip]
static AC_QLOOKUP_12_QTX: [i16; 256] = [
...
/// `av1_dc_quant_QTX(qindex, delta, bit_depth)` — the DC quantizer step.
///
/// `bit_depth` must be 8, 10, or 12; any other value returns `-1` (mirroring the
/// C behaviour with `NDEBUG` — the `assert` is compiled out in the release
/// oracle build). `qindex + delta` is clamped to `[0, MAXQ]`.
#[inline]
pub fn av1_dc_quant_qtx(qindex: i32, delta: i32, bit_depth: u8) -> i16 {
    let q = (qindex + delta).clamp(0, MAXQ) as usize;
    match bit_depth {
        8 => DC_QLOOKUP_QTX[q],
        10 => DC_QLOOKUP_10_QTX[q],
        12 => DC_QLOOKUP_12_QTX[q],
        _ => -1,
    }
}

/// `av1_ac_quant_QTX(qindex, delta, bit_depth)` — the AC quantizer step.
///
/// Same conventions as [`av1_dc_quant_qtx`].
#[inline]
pub fn av1_ac_quant_qtx(qindex: i32, delta: i32, bit_depth: u8) -> i16 {
    let q = (qindex + delta).clamp(0, MAXQ) as usize;
    match bit_depth {
        8 => AC_QLOOKUP_QTX[q],
        10 => AC_QLOOKUP_10_QTX[q],
        12 => AC_QLOOKUP_12_QTX[q],
        _ => -1,
    }
}
