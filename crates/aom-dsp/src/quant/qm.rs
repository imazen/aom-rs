...
//! ported verbatim from libaom (generated into [`crate::quant::qm_fwd_tables`]);
...
use crate::quant::qm_fwd_tables::WT_MATRIX_REF;
...
/// [`crate::quant::qm_inv_tables`]). The encoder needs the inverse alongside the
...
pub fn iqmatrix(
    ...
    let off = QM_OFFSET[tx_size];
    let len = QM_LEN[tx_size];
    Some(&crate::quant::qm_inv_tables::IWT_MATRIX_REF[qm_level][c][off..off + len])
}
